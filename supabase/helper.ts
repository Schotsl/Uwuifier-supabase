...
export async function fetchPlausible(metric: string) {
  ...
  const plausibleUrl =
    `${plausibleHost}/api/v1/stats/breakdown?site_id=${plausibleDomain}&period=12mo&property=event:props:method&filters=event:name%3D%3D${encodedMetric}&metrics=events`;
